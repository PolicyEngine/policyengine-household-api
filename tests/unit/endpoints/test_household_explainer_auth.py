...
class TestAnthropicConfiguration:
    ...
    def test__given_ai_disabled_and_no_api_key__returns_401(
        self, mock_flask_app, mock_ai_disabled_no_key, test_household_payload
    ):
    ...
    def test__given_backward_compatible_env_var__returns_true(
        self, mock_backward_compatibility_env_key
    ):
...
class TestHouseholdExplainerAuth:
    ...
    def test__given_ai_enabled_but_no_api_key__returns_401(
        self, mock_flask_app, mock_ai_enabled_no_key, test_household_payload
    ):
    ...
    def test__given_api_key_in_env_var__auto_enables_ai(
        self,
        mock_flask_app,
        mock_backward_compatibility_env_key,
        mock_household_model_us,
        ...
        with mock_flask_app.test_request_context(json=test_household_payload):
            ...
            )  # Bad request due to no null variables
            data = json.loads(response.data)
            assert "at least one variable set to null" in data["message"]

    def test__given_ai_enabled_with_api_key__proceeds_with_request(
        self,
        mock_flask_app,
        mock_ai_enabled_with_key,
        mock_household_model_us,
