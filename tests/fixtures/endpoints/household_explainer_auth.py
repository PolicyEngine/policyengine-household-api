    ...
    with patch(
        ...
        with patch("os.getenv") as mock_getenv:
            ...
            mock_getenv.return_value = (
                "sk-ant-env-key"  # But key exists in env var
            )
...
def mock_household_model_us():
    ...
    with patch(
        "policyengine_household_api.endpoints.household_explainer.HouseholdModelUS"
    ) as mock_model:
...
def mock_flatten_variables_empty():
    ...
    with patch(
        "policyengine_household_api.endpoints.household_explainer.flatten_variables_from_household"
    ) as mock_flatten:
...
def mock_google_cloud_storage_not_found():
    ...
    with patch(
        "policyengine_household_api.endpoints.household_explainer.GoogleCloudStorageManager"
    ) as mock_storage:
        mock_storage_instance = MagicMock()
        mock_storage_instance.get.side_effect = FileNotFoundError(
            "Test UUID not found"
        )
...
def test_household_payload():
    ...
    return {
        ...
        "household": {"people": {"person1": {"age": {"2024": 30}}}},
    }
...
def test_household_payload_with_null():
    ...
    return {
        "computation_tree_uuid": "test-uuid",
        "household": {"people": {"person1": {"age": {"2024": None}}}},
    }
