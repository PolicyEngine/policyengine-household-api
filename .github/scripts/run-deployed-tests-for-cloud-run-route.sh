...
channel="${1:?Usage: run-deployed-tests-for-cloud-run-route.sh CHANNEL ROUTE_MODE}"
route_mode="${2:?Usage: run-deployed-tests-for-cloud-run-route.sh CHANNEL ROUTE_MODE}"
...
if [ -z "${base_url}" ]; then
  echo "HOUSEHOLD_API_BASE_URL must be set" >&2
  exit 1
fi
...
# report instead of ending the step before any test output.
echo "Running deployed tests through Cloud Run for ${channel} via ${route_mode} routing"
